...
import { Alert, Card, CardContent, FormControl, FormHelperText, Select,MenuItem, InputAdornment, InputLabel, TextField, Typography } from '@mui/material'
...
function StudentTransfer() {
    ...
    React.useEffect(() => {
        ...
        axios.get('https://smart-india-hackathon-server.vercel.app/api/fetchalluniversity').then(res => {
            ...
            setError(true)
            setHelperText("We are facing some technical issues. Please try again later.")
        })
    } , [])
    ...
    const [presentUniversityError, setPresentUniversityError] = React.useState(false)
    ...
    const handlePresentUniversity = async(aadharNumber) => {
        ...
        if(studentEntry.data.code==="success")
        {
            ...
            setHelperText("No record exist with this aadhar. Please enter a valid aadhar")
        }
        else
        {
    ...
    const handleSubmit = async(e) => {
        ...
        const studentTransfer = await axios.post("https://smart-india-hackathon-server.vercel.app/api/transferstudent", {
            headers : {
                'Content-Type': 'application/json',
                "access-control-allow-origin": "*"
            },
                ...
                <React.Fragment>
                    <CardContent>
                        ...
                        {warning ? <Alert severity="warning">{helperText}</Alert> : null}
                            <br/>
                            <br/>
                        <Typography className="text-center" variant='h4'>Student Transfer</Typography>
